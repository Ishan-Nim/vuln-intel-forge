        ...
        <div className="flex items-center gap-2">
          <Link to="/" className="flex items-center gap-2">
            <img 
              ...
              alt="DNA Logo" 
              className={cn("w-auto", isDrVulner01Page ? "h-8" : "h-10")}
            />
            <span className={cn(
              "hidden font-bold md:inline-block",
              isDrVulner01Page ? "text-lg" : "text-xl"
            )}>Dr.Vulner0/1</span>
          </Link>
        </div>
  ...
  return (
    <Link 
      ...
      className={cn(
        ...
        isDrVulner01Page ? "px-3 py-1.5" : "px-4 py-2",
        isActive 
    ...
    >
      ...
      <span className={cn(
        "font-medium",
        isDrVulner01Page ? "text-xs" : "text-sm"
      )}>{children}</span>
      {isActive && (
...
const MobileNavToggle: React.FC = () => {
  ...
  return (
    <Button 
      ...
      size="icon" 
      className="md:hidden"
      style={isDrVulner01Page ? { width: '32px', height: '32px' } : {}}
    >
      <span className="sr-only">Toggle Menu</span>
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width={isDrVulner01Page ? "20" : "24"}
        height={isDrVulner01Page ? "20" : "24"}
        viewBox="0 0 24 24"
        fill="none"
        stroke="currentColor"
        strokeWidth="2"
